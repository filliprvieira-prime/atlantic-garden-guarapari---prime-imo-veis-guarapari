...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Verificar se deve abrir painel admin
  ...
  const openLightbox = (imageUrl: string, title: string) => {
    ...
    trackLightboxOpen('galeria', title);
  };

  // Handler de WhatsApp com tracking
  const handleWhatsAppClick = async (location: string) => {
    trackButtonClick('whatsapp', location);
    await trackWhatsAppClick(location);
  };

  // Handler de navegação com tracking
  const handleNavClick = (section: string) => {
    trackNavigationClick(section);
  };

  // Handler de item de lazer
  const handleLeisureClick = (itemName: string) => {
    trackLeisureItemClick(itemName);
  };
  ...
  // Admin: Carregar leads
  const loadLeads = async () => {
    setLoadingLeads(true);
    const recentLeads = await getRecentLeads(50);
    setLeads(recentLeads);
    setLoadingLeads(false);
  };

  // Admin: Buscar por código
  const handleSearchByCode = async () => {
    ...
    setLoadingLeads(true);
    const results = await searchLeadsByCode(searchCode);
    setLeads(results);
    setLoadingLeads(false);
  };
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden selection:bg-[var(--secondary-color)] selection:text-white font-['Inter']">
      ...
      <nav className="fixed w-full z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-8 items-center font-bold text-xs uppercase tracking-widest text-slate-500">
            ...
            <a href="#infra" onClick={() => handleNavClick('infra')} className="hover:text-custom-primary transition-colors">Infra</a>
            <a href="#localizacao" onClick={() => handleNavClick('localizacao')} className="hover:text-custom-primary transition-colors">Localização</a>
            <a href={PAGE_CONFIG.contact.directWhatsappLink} onClick={() => handleWhatsAppClick('nav')} target="_blank" rel="noopener noreferrer" className="bg-custom-primary text-white px-6 py-3 rounded-full hover:bg-custom-secondary transition-all">Quero Saber Mais</a>
          </div>
        </div>
      </nav>
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href={PAGE_CONFIG.contact.directWhatsappLink} onClick={() => handleWhatsAppClick('hero')} target="_blank" rel="noopener noreferrer" className="bg-white text-custom-primary px-10 py-5 rounded-full text-sm font-black uppercase tracking-widest hover:bg-custom-secondary hover:text-white transition-all shadow-2xl shadow-black/20 text-center">
                {PAGE_CONFIG.hero.ctaPrimary}
              </a>
              <a href="#lazer" onClick={() => handleNavClick('lazer')} className="bg-custom-primary/30 backdrop-blur-md border border-white/30 text-white px-10 py-5 rounded-full text-sm font-black uppercase tracking-widest hover:bg-white/10 transition-all text-center">
        ...
        <div className="flex flex-col gap-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {PAGE_CONFIG.lazer.galleryDriveIds.map((id, idx) => (
              <div 
                key={idx} 
                className="relative rounded-[40px] overflow-hidden shadow-xl aspect-[4/3] bg-white group hover:shadow-2xl transition-all duration-500 cursor-pointer"
                onClick={() => {
                  trackGalleryView(`Lazer ${idx + 1}`);
                  openLightbox(getDriveUrl(id), `Lazer ${idx + 1}`);
                }}
              >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {AMENITIES.map((amenity, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className="group p-6 bg-white border border-slate-100 rounded-[35px] text-center hover:bg-custom-primary transition-all duration-500 shadow-sm hover:shadow-lg cursor-pointer"
                  onClick={() => handleLeisureClick(amenity.title)}
                >
      ...
      <footer className="bg-white py-20 px-6 border-t border-slate-100 text-center">
        ...
        <p className="text-[10px] text-slate-400 mt-4">Seu código: <span className="font-bold text-custom-secondary">{leadCode}</span></p>
      </footer>
      ...
      {lightboxOpen && (
        ...
        >
          ...
          <div className="absolute top-6 left-6 bg-custom-primary/90 px-4 py-2 rounded-full">
            <span className="text-white text-sm font-bold">Atlantic Garden</span>
          </div>
          <div className="max-w-[90vw] max-h-[90vh]" onClick={e => e.stopPropagation()}>
            ...
            />
            <p className="text-center text-white/70 mt-4 text-sm font-medium">{lightboxTitle}</p>
          </div>
        </div>
      )}
      ...
      {/* Admin Panel */}
      {showAdmin && (
        <div className="fixed inset-0 z-[200] bg-slate-900/95 backdrop-blur-xl overflow-auto">
          <div className="max-w-6xl mx-auto p-8">
            <div className="flex justify-between items-center mb-8">
              <h1 className="text-3xl font-black text-white">Painel Admin - Atlantic Garden</h1>
              <button 
            ...
            {!isAdminAuth ? (
              <div className="max-w-md mx-auto bg-slate-800 p-8 rounded-2xl">
                <h2 className="text-xl font-bold text-white mb-6">Digite a senha</h2>
                <input
                  type="password"
                  value={adminPassword}
                  onChange={e => setAdminPassword(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && handleAdminLogin()}
                  className="w-full px-4 py-3 bg-slate-700 text-white rounded-xl mb-4"
                  placeholder="Senha do admin"
                />
                <button
                  onClick={handleAdminLogin}
                  className="w-full py-3 bg-custom-primary text-white rounded-xl font-bold hover:bg-custom-secondary transition-colors"
                >
                  Entrar
                </button>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-4 mb-8">
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-3 bg-slate-800 text-white rounded-xl"
                      placeholder="Buscar por código (ex: A3B7X2)"
                    />
                  </div>
                  <button
                    onClick={handleSearchByCode}
                    className="px-6 py-3 bg-custom-primary text-white rounded-xl font-bold hover:bg-custom-secondary transition-colors"
                  >
                    Buscar
                  </button>
                  <button
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-slate-800 rounded-2xl p-6 max-h-[70vh] overflow-auto">
                      ...
                      <div className="space-y-3">
                        {leads.map((lead, idx) => (
                          <div
                            ...
                            className={`p-4 rounded-xl cursor-pointer transition-colors ${selectedLead?.id === lead.id ? 'bg-custom-primary' : 'bg-slate-700 hover:bg-slate-600'}`}
                          >
                            <div className="flex justify-between items-start">
                              <div>
                                <p className="font-bold text-white">{lead.leadCode}</p>
                                <p className="text-sm text-slate-300">{lead.nomeCliente || 'Sem nome'}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-xs text-slate-400">{lead.device}</p>
                                {lead.isReturningVisitor && (
                                  <span className="text-xs bg-green-500 text-white px-2 py-1 rounded-full">Retorno</span>
                                )}
                              </div>
                            </div>
                            <div className="flex gap-4 mt-2 text-xs text-slate-400">
                              <span className="flex items-center gap-1"><Clock size={12} /> {lead.totalTimeFormatted}</span>
                              <span className="flex items-center gap-1"><Target size={12} /> {lead.resumo?.nivelInteresse || 0}%</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {selectedLead ? (
                      <div className="space-y-4">
                        <div className="bg-slate-700 p-4 rounded-xl">
                          ...
                          <p className="text-2xl font-black text-custom-secondary">{selectedLead.leadCode}</p>
                        </div>
                        
                        {selectedLead.nomeCliente && (
                          <div className="bg-slate-700 p-4 rounded-xl">
                            <p className="text-xs text-slate-400 mb-1">Cliente</p>
                            <p className="text-white font-bold">{selectedLead.nomeCliente}</p>
                            <p className="text-slate-300">{selectedLead.telefoneCliente}</p>
                          </div>
                        )}

                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-slate-700 p-4 rounded-xl">
                            <p className="text-xs text-slate-400 mb-1">Tempo no Site</p>
                            <p className="text-white font-bold">{selectedLead.totalTimeFormatted}</p>
                          </div>
                          <div className="bg-slate-700 p-4 rounded-xl">
                            <p className="text-xs text-slate-400 mb-1">Score de Interesse</p>
                            <p className="text-white font-bold">{selectedLead.resumo?.nivelInteresse || 0}%</p>
                          </div>
                        </div>

                        <div className="bg-slate-700 p-4 rounded-xl">
                          <p className="text-xs text-slate-400 mb-1">Dispositivo</p>
                          <p className="text-white">{selectedLead.device} • {selectedLead.browser}</p>
                          <p className="text-slate-400 text-sm">{selectedLead.screenSize}</p>
                        </div>

                        <div className="bg-slate-700 p-4 rounded-xl">
                          <p className="text-xs text-slate-400 mb-1">Origem</p>
                          <p className="text-white">{selectedLead.utmSource || selectedLead.referrer || 'Acesso direto'}</p>
                        </div>

                        {selectedLead.leisureItemsClicked?.length > 0 && (
                          <div className="bg-slate-700 p-4 rounded-xl">
                            <p className="text-xs text-slate-400 mb-2">Itens de Lazer Clicados</p>
                            <div className="flex flex-wrap gap-2">
                              {selectedLead.leisureItemsClicked.map((item: string, i: number) => (
                                <span key={i} className="bg-custom-secondary/20 text-custom-secondary px-2 py-1 rounded-lg text-xs">{item}</span>
                              ))}
                            </div>
                          </div>
                        )}
                        ...
                        {selectedLead.timeline?.length > 0 && (
                          <div className="bg-slate-700 p-4 rounded-xl">
                            <p className="text-xs text-slate-400 mb-2">Timeline de Ações</p>
                            <div className="space-y-2 max-h-48 overflow-auto">
                              {selectedLead.timeline.slice(-10).reverse().map((event: any, i: number) => (
                                <div key={i} className="flex items-start gap-2 text-sm">
                                  <ChevronRight size={14} className="text-custom-secondary mt-1" />
                                  <div>
                      ...
                      <p className="text-slate-400">Selecione um lead para ver detalhes</p>
                    )}
                  </div>
                </div>
              </div>
